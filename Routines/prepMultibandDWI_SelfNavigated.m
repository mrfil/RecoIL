function prepMultibandDWI_SelfNavigated(datadir,filename)
% function prepMultibandDWI_SelfNavigated(datadir,filename)
%
% Inputs (optional):
%
...
if (exist('sen.mat','file') ~= 2)
    ...
    save -v7.3 sen.mat  senMB
    save -v7.3 FM.mat   FMMB
    save -v7.3 mask.mat maskMB
else 
    load sen.mat;
    load FM.mat;
    load mask.mat;
...
for shot = 1:rInfo.nShots
    shotFilename = [filename '_Shot_' num2str(shot)  '.h5'];
    if rInfo.nCoils < 32
        convertRecoInfoToIsmrmrd(shotFilename, rInfo, senMB, FMMB, [], 'shotsToRecon', shot);
    else
...
%convertRecoInfoToIsmrmrd(sprintf('%s.h5',filename),rInfo,permute(senMB,[1 2 3 5 4]), FMMB, [] );
