function [img, resid, roughness] = phaseCorrectedReconMag(rInfo, sen, mask, FM, PMaps, magErrors, varargin)
         ...
         for mm = 1:length(phasesToRecon)
            for nn = 1:length(echoesToRecon)
               for oo = 1:length(repetitionsToRecon)
                  ...
                  M = reshape(magErrors(:,:,:,:,:,slc,avg,phs,eco,rep),rInfo.N,rInfo.N,Nz,rInfo.nShots*rInfo.nPartitions);
                  % S = pcSENSE(A,P,sen_tmp(logical(col(mask(:,:,:,kk))),:));
                  S = pcSENSE_magErrors(A,P,M,sen_tmp(logical(col(mask(:,:,:,slc))),:)); % CLJ: changing this to slc, instead of the index kk, for situations where slc ~= kk
                  data = col(data);
