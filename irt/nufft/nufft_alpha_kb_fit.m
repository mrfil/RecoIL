...
%% Sometimes (for N = 2) these matrices can be singular inserting NaNs.
if(sum(~isfinite((X \ sn_kaiser)')) == 0) % No NaNs or infs in the coefs
    coef = (X \ sn_kaiser)';	% regress(sn_kaiser, X)';
else % Use a more robust(?) option
