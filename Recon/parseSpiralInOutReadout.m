function obj = parseSpiralInOutReadout(obj)

girfTs = obj.GIRFts;

% To Do: deal with two-shot mode.
SameShotMode = true;
...
kreadi = vertcat(col(kreadi(end:-1:1)),col(kreadi));
kphasei = vertcat(col(kphasei(end:-1:1)),col(kphasei));
Greado = vertcat(col(-1*Greado(end:-1:1)),col(Greado));
Gphaseo = vertcat(col(-1*Gphaseo(end:-1:1)),col(Gphaseo));
...
for ii = 0:(obj.nShotsUsed-1)
    ang_rot = phi*(ii-(obj.nShotsUsed-1)*floor(ii/obj.nShotsUsed));
    obj.kReadNominal(:,ii+1) = kreadN*cos(ang_rot) + kphaseN*sin(ang_rot);
    obj.kPhaseNominal(:,ii+1) = kphaseN*cos(ang_rot) - kreadN*sin(ang_rot);
end
...
for ii = 0:(obj.nShotsUsed-1)
    if obj.useGIRF
        Gxc(:,ii+1) = conv(Gx(:,ii+1),obj.GIRF(:,1),'same');
        Gyc(:,ii+1) = conv(Gy(:,ii+1),obj.GIRF(:,2),'same');
        Gzc(:,ii+1) = conv(Gz(:,ii+1),obj.GIRF(:,3),'same');
    else
        Gxc(:,ii+1) = Gx(:,ii+1);
        Gyc(:,ii+1) = Gy(:,ii+1);
        Gzc(:,ii+1) = Gz(:,ii+1);
...
if obj.nPartitions > 1
    ...
    %kSlicec = (-obj.nPartitions/2+1):(obj.nPartitions/2);
...
for ii = 1:(obj.nShotsUsed)
    kPhaset(:,ii) = cumsum([Gphasec(:,ii)])*girfTs*obj.FOV*gambar;
    kReadt(:,ii) = cumsum([Greadc(:,ii)])*girfTs*obj.FOV*gambar;
...
obj.shotLength = length(obj.kReadNominal(:,1)); %Assume kxt and kyt are the same length

if obj.useGIRF
    obj.kRead = repmat(kReadc(1:obj.shotLength,:),[1,1,obj.nPartitions, obj.nSlices,obj.nAverages, obj.nPhases, obj.nEchoes, obj.nRepetitions,1]);
    obj.kPhase = repmat(kPhasec(1:obj.shotLength,:),[1,1,obj.nPartitions, obj.nSlices,obj.nAverages, obj.nPhases, obj.nEchoes, obj.nRepetitions,1]);
    ...
    obj.kRead = repmat(obj.kReadNominal(1:obj.shotLength,:),[1,1,obj.nPartitions, obj.nSlices,obj.nAverages, obj.nPhases, obj.nEchoes, obj.nRepetitions,1]);
    obj.kPhase = repmat(obj.kPhaseNominal(1:obj.shotLength,:),[1,1,obj.nPartitions, obj.nSlices,obj.nAverages, obj.nPhases, obj.nEchoes, obj.nRepetitions,1]);
    kSlicec = permute(kSlicec,[3,1,2]);
    obj.kSlice = repmat(kSlicec,[length(kReadc(1:obj.shotLength,1)),obj.nShots,1, obj.nSlices,obj.nAverages, obj.nPhases, obj.nEchoes, obj.nRepetitions,1]);
...
%obj.ww = weight_vor(col(obj.kRead(:,:,1,1,1,1,1,1,1)),col(obj.kPhase(:,:,1,1,1,1,1,1,1)),obj.nShotsDesigned,1);
%obj.ww(obj.ww > 2) = 2;
%obj.ww = reshape(obj.ww,[],obj.nShots);
obj.ww = calc3DDensityCompensation(col(obj.kRead(:,:,1,1,1,1,1,1,1)),col(obj.kPhase(:,:,1,1,1,1,1,1,1)),col(obj.kSlice(:,:,1,1,1,1,1,1,1)),obj.N,obj.nPartitions);
obj.ww = reshape(obj.ww,size(obj.kRead(:,:,1,1,1,1,1,1,1)));
