function obj = parse_pgseMBc(obj)
   ...
   [grads_mTm, FOVRef, FOVzRef, RUP] = readExternalGradFile([path '/trajectories/Rxy4Rz1N60Nz4/Rxy4Rz1Nxy60Nz4.xml']);
   RUP
   obj.adcTsNav = obj.DataObject.hdr.Dicom.DICOM.alICEProgramPara(9)*1E-6;
   kspace = calcKSpaceFromGrads(obj, grads_mTm, obj.adcTsNav, FOVRef, 'FOVz', FOVzRef, 'RUP', RUP, 'UseGIRF', obj.useGIRF );
   obj.ptsToDropNav = obj.DataObject.hdr.Dicom.DICOM.alICEProgramPara(8) + ((RUP*10e-6)/obj.adcTsNav);
   % need to add the RUP points to obj.ptsToDrop
   kRead_nav = kspace(:,1);
   kPhase_nav = kspace(:,2);
   kSlice_nav = kspace(:,3);
   ...
   obj.timingVecNav = -1*col(flip(0:obj.adcTsNav:obj.adcTsNav*(length(obj.kReadNav(:,1))-1)));
   ...
   if obj.multibandFactor > 1
      obj.halfVoxelShiftNavSlice = -1/(4*obj.multibandFactor); % CLJ: making this positive, since that is what works
   else
