function obj = parseExtNavTest(obj)
...
for ii = 0:(obj.nShotsUsed-1)
    for jj = 1:length(Greadi)
        temp = obj.rotMatrix*[Gread(jj,ii+1);Gphase(jj,ii+1);Gslice(jj,ii+1)];
        Gx(jj,ii+1) = temp(1);
        Gy(jj,ii+1) = temp(2);
        Gz(jj,ii+1) = temp(3);
    end
...
for ii = 1:(obj.nShotsUsed)
    ...
    kzt(:,ii) = cumsum([0; Gslicec(:,ii)])*girfTs*obj.FOV/2*gambar;
end
...
for ii = 1:(obj.nShotsUsed)
    ...
    obj.kxNominal(:,ii) = interp1(0:obj.gradTs:obj.gradTs*length(kspace(:,1))-obj.gradTs,kspace(:,1)*obj.FOV,0:obj.adcTs:obj.gradTs*length(kspace(:,1))-obj.adcTs,'linear','extrap')';
    obj.kyNominal(:,ii) = interp1(0:obj.gradTs:obj.gradTs*length(kspace(:,2))-obj.gradTs,kspace(:,2)*obj.FOV,0:obj.adcTs:obj.gradTs*length(kspace(:,2))-obj.adcTs,'linear','extrap')';
    obj.kzNominal(:,ii) = interp1(0:obj.gradTs:obj.gradTs*length(kspace(:,3))-obj.gradTs,kspace(:,3)*obj.FOV/2,0:obj.adcTs:obj.gradTs*length(kspace(:,3))-obj.adcTs,'linear','extrap')';
end
...
obj.timingVec = 0:obj.adcTs:obj.adcTs*(length(obj.kxNav)-1);
