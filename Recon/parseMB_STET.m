function obj = parseMB_STET(obj)
   %parseMB_STEa.m - Parsing object for 2D/3D multiband STEAM Diffusion acquisitions.
   %
   % Syntax:  [obj] = parseMB_STEa(obj)
   %
   % Inputs:
   ...
   % Deal with ExcOrder  - Because this sequence is 3D it appears that the
   % slice order is flipped relative to the 2D sequences. Why this occurs
   % is not clear.
   %obj.ExcOrder = [obj.nSlices:-1:1];
