...
if (type == 2) % Approx Histogram Interpolator using we_histo
     ...
     % Can't use the existing histogram interpolator code if KK < 2L+1
     % Fall back to exact interpolator to avoid erroring out
     % AMC 6/19/2018
     if (KK < (2*L+1))
         sprintf('int_tim_seg: KK < 2*L+1, falling back to exact interpolator.')
