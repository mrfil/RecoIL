 function [xs, info, norms, rough] = solve_pwls_pcg(x, A, W, yi, R, varargin)
...
% Edit 2015_07_15 JLH: edit regularization to use R.C1 and potential function instead of R.denom
% Edit 2019_02_18 AMC: modified to eliminate R.C1 and use modified Robj and
% put back to R.Denom.
...
if nargout > 2
    norms = zeros(arg.niter,1);
end
if nargout > 3
    rough = zeros(arg.niter,1);
end
...
for iter = 1:arg.niter
    ...
    % Calculating norm of grad for L curve usage
    if nargout > 2
        norms(iter) = norm(yi - Ax,2);
    end
    if nargout > 3
        rough(iter) = R.penal(x);
    end
    
	% (negative) gradient
	ngrad = A' * (W .* (yi-Ax));
	pgrad = R.cgrad(x);
%     pgrad = R.C1'*(R.C1*x.*R.wt);
...
%         step = real((ddir' * ngrad1) / ldenom-(ddir' * pgrad) / pdenom);
...
% 		dAWAd = reale(dAWAd); % 2008-10-16
		...
		for is=1:nsub
% 			pdenom = Cdir' * (R.wpot(R.wt, Cdir) .* Cdir); % avoid Cdir 
%            pdenom = Cdir' * (R.pot.wpot(R.pot,(R.C1*(x + step * ddir)).*(R.wt>0)) .* Cdir); % 2015_07_13
%			pdenom = (abs(ddir).^2)' * R.denom(R, x + step * ddir);
% 			pdenom = dot_double(abs(ddir).^2, R.denom(R, x + step * ddir));
%             pdenom = (R.C1*ddir)'*(R.C1*ddir);
